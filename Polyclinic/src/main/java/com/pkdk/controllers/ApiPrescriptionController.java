...
public class ApiPrescriptionController {
    ...
    public ResponseEntity<?> createPrescription(@PathVariable("recordId") int recordId,
        ...
        if (AppointmentStatus.COMPLETED.name().equals(m.getAppointmentId().getStatus())) {
            return new ResponseEntity<>("Không thể kê đơn thuốc khi lịch hẹn đã hoàn thành", HttpStatus.FORBIDDEN);
        }
    ...
    public ResponseEntity<?> createPrescriptionPayment(Principal principal, @PathVariable("prescriptionId") int prescriptionId) {
        ...
        try {
            Payments payment = this.prescriptionService.createPayment(prescriptionId);
